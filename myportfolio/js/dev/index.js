...
const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.attributeName === "class") {
      ...
      if (target.classList.contains("--watcher-view")) {
        const sectionId = target.getAttribute("id");
        if (sectionId) {
          const activeLink = document.querySelector(`.menu__link[href="#${sectionId}"]`);
          if (activeLink) {
...
const sections = document.querySelectorAll("[id][data-fls-watcher]");
sections.forEach((section) => {
  observer.observe(section, { attributes: true });
});
function scrollAction() {
  const header2 = document.querySelector(".header");
  header2.classList.toggle("header--scroll", scrollY > 20);
}
